...
# Resource-3: Azure MySQL Firewall Rule - Allow access from web_linuxvm Host Public IP
resource "azurerm_mysql_firewall_rule" "mysql_fw_rule" {
  name                = "allow-access-from-web-linuxvm-publicip"
  resource_group_name = azurerm_resource_group.rg.name
  server_name         = azurerm_mysql_server.mysql_server.name
  start_ip_address    = azurerm_public_ip.web_linuxvm_publicip.ip_address
  end_ip_address      = azurerm_public_ip.web_linuxvm_publicip.ip_address
}
